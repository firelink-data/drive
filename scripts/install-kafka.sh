...
env KAFKA_LIB_PATH=$KAFKA_LIB_PATH KAFKA_COMBINED_LOG_PATH=$KAFKA_COMBINED_LOG_PATH KAFKA_METADATA_LOG_PATH=$KAFKA_METADATA_LOG_PATH sudo -Eu kafka bash -c 'mkdir -p $KAFKA_LIB_PATH; mkdir -p $KAFKA_COMBINED_LOG_PATH; mkdir -p /var/lib/kafka/metadata-logs; chown -R kafka:kafka /var/lib/kafka'
